...
public class WinController {
    ...
    private void ensureAdminExists() throws SQLException {
        try (Connection conn = DB.getDbConnection()) {
            // Создадим таблицу users если её нет (SQLite синтаксис)
            try (Statement st = conn.createStatement()) {
                st.execute("CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, login TEXT UNIQUE NOT NULL, Email TEXT, Password TEXT)");
            }
